...
FNEB::FNEB(LAMMPS *lmp) : Pointers(lmp) {}

/* ----------------------------------------------------------------------
   internal FNEB constructor, called from TAD
------------------------------------------------------------------------- */

FNEB::FNEB(LAMMPS *lmp, double etol_in, double ftol_in, int n1steps_in,
...
/* ----------------------------------------------------------------------
   perform FNEB on multiple replicas
------------------------------------------------------------------------- */

void FNEB::command(int narg, char **arg)
{
  if (domain->box_exist == 0)
    error->all(FLERR,"FNEB command before simulation box is defined");

  if (narg < 6) error->universe_all(FLERR,"Illegal FNEB command");
  ...
  if (etol < 0.0) error->all(FLERR,"Illegal FNEB command");
  if (ftol < 0.0) error->all(FLERR,"Illegal FNEB command");
  if (nevery == 0) error->universe_all(FLERR,"Illegal FNEB command");
  if (n1steps % nevery || n2steps % nevery)
    error->universe_all(FLERR,"Illegal FNEB command");
  ...
  if (nreplica == 1) error->all(FLERR,"Cannot use FNEB with a single replica");
  if (nreplica != universe->nprocs)
    error->all(FLERR,"Can only use FNEB with 1-processor replicas");
  if (atom->sortfreq > 0)
    error->all(FLERR,"Cannot use FNEB with atom_modify sort enabled");
  if (atom->map_style == 0)
    error->all(FLERR,"Cannot use FNEB unless atom map exists");

  // process file-style setting to setup initial configs for all replicas

  if (strcmp(arg[5],"final") == 0) {
    if (narg != 7) error->universe_all(FLERR,"Illegal FNEB command");
    infile = arg[6];
    readfile(infile,0);
    ...
    if (narg != 6) error->universe_all(FLERR,"Illegal FNEB command");
  } else error->universe_all(FLERR,"Illegal FNEB command");

  // run the FNEB calculation

  run();
}

/* ----------------------------------------------------------------------
   run FNEB on multiple replicas
------------------------------------------------------------------------- */
...
void FNEB::run()
{
  ...
  for (ineb = 0; ineb < modify->nfix; ineb++)
    if (strcmp(modify->fix[ineb]->style,"fneb") == 0) break;
  if (ineb == modify->nfix) error->all(FLERR,"FNEB requires use of fix fneb");

  fneb = (FixFNEB *) modify->fix[ineb];
  nall = 4;
  ...
  if (update->minimize->searchflag)
    error->all(FLERR,"FNEB requires damped dynamics minimizer");

  // setup regular FNEB minimization

  if (me_universe == 0 && universe->uscreen)
    fprintf(universe->uscreen,"Setting up regular FNEB ...\n");
  ...
  if (update->laststep < 0 || update->laststep > MAXBIGINT)
    error->all(FLERR,"Too many timesteps for FNEB");
  ...
  // perform regular FNEB for n1steps or until replicas converge
  // retrieve PE values from fix FNEB and print every nevery iterations
  ...
  // switch fix FNEB to climbing mode
  ...
  // setup climbing FNEB minimization
  ...
  // perform climbing FNEB for n2steps or until replicas converge
  // retrieve PE values from fix FNEB and print every nevery iterations
  // break induced if converged
  // damped dynamic min styles insure all replicas converge together
...
void FNEB::readfile(char *file, int flag)
{
  ...
  if (me_universe == 0 && screen)
    fprintf(screen,"Reading FNEB coordinate file(s) ...\n");
...
/* ----------------------------------------------------------------------
   universe proc 0 opens FNEB data file
   test if gzipped
------------------------------------------------------------------------- */

void FNEB::open(char *file)
{
  compressed = 0;
...
/* ----------------------------------------------------------------------
   query fix FNEB for info on each replica
   proc 0 prints current FNEB status
------------------------------------------------------------------------- */

void FNEB::print_status()
{
