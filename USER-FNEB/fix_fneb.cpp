...
FixFNEB::FixFNEB(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  for (int iarg = 4; iarg < narg; ++iarg) {
    if (strcmp(arg[iarg], "delta") == 0){
      target_energy_delta = force->numeric(FLERR,arg[iarg+1]);
      ++iarg;
    } else if (strcmp(arg[iarg], "prefactor") == 0){
      relax_prefactor = force->numeric(FLERR,arg[iarg+1]);
      ++iarg;
    }
  }
...
void FixFNEB::init()
{
  ...
  if (ireplica == nreplica - 1){
    target_energy = veng + target_energy_delta;
  }
...
void FixFNEB::min_setup(int vflag)
{
...
void FixFNEB::min_post_force(int vflag)
{
  ...
  // Do quasi-Newton step for the last replica
  if (ireplica == nreplica - 1) reset_final_replica();
  ...
  // ----------------FNEB-----------------------
  // now we will change the force of the last node
  if (ireplica == 0 ) {
    plen = nlen = 0.0;
    return;
  }
  ...
  else if (ireplica == nreplica - 1) prefactor = -dot;
  else prefactor = -dot + kspring*(nlen-plen);


  // Here what I use is a little diferent from the origin free end NEB method ( Ting Zhu, 2007, PNAS, 104)
  // The force on the last replica F = (-Grad(V)_perp) - (-Grad(V)_perp) * Grad(V) / |Grad(V)|.
  if (ireplica == nreplica - 1){
    double flen = 0.;
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
	flen += f[i][0]*f[i][0] + f[i][1]*f[i][1] + f[i][2]*f[i][2];
      }
    }
    flen = sqrt(flen);
    double flen_inv = 1. / flen;
    double newdot = 0.;
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
	newdot += (f[i][0] + prefactor*tangent[i][0]) * f[i][0];
	newdot += (f[i][1] + prefactor*tangent[i][1]) * f[i][1];
	newdot += (f[i][2] + prefactor*tangent[i][2]) * f[i][2];
      }
    }
    newdot = newdot * flen_inv;
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
	f[i][0] = (f[i][0] + prefactor * tangent[i][0]) * (1 - newdot * f[i][0] * flen_inv);
	f[i][1] = (f[i][1] + prefactor * tangent[i][1]) * (1 - newdot * f[i][1] * flen_inv);
	f[i][2] = (f[i][2] + prefactor * tangent[i][2]) * (1 - newdot * f[i][2] * flen_inv);
      }
    }
  }
  else {
...
void FixFNEB::reset_final_replica(){
  double **x = atom->x;
  double **f = atom->f;
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
  ...
  flen = sqrt(flen);
  double prefactor = relax_prefactor * (veng - target_energy) / flen / flen;
  for (int i = 0; i < nlocal; ++i) {
    if (mask[i] & groupbit) {
      x[i][0] += prefactor * f[i][0];
      x[i][1] += prefactor * f[i][1];
      x[i][2] += prefactor * f[i][2];
    }
  }
}
